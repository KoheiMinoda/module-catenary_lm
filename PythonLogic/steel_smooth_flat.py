...
for k in range(num_segs):
    ...
    print(f"セグメント {k}-{k+1}: Chain, EA={EA_CHAIN:.2e}, CA={CA_CHAIN:.2e}")
...
def axial_forces(nodes, segments):
    ...
    tensions = [0.0] * len(segments)  # 各セグメントの張力を記録
...
# ---- 海底制約の適用（位置・速度の修正） ---------------------------------
def apply_seabed_constraint(nodes):
    """
    海底面より下にあるノードを海底面に修正し、
    下向きの速度成分を削除する
    """
    for k, nd in enumerate(nodes):
        ...
        # 位置制約：海底面より下にある場合は海底面に移動
        if z < seabed_z_local:
            nd["pos"][2] = seabed_z_local
            
            # 下向きの速度成分を削除（上向きの速度は保持）
            if vz < 0.0:
                nd["vel"][2] = 0.0
...
# 張力データの記録用
tension_data = []  # [time, fairleader_tension, anchor_tension] 
...
while t <= T_END:
    ...
    a_list, segment_tensions = compute_acc(nodes, segments)
    ...
    if step_count % output_interval == 0:
        ...
        # 張力データを記録
        # フェアリーダー張力：セグメント0（FP-ノード1間）
        # アンカー張力：セグメント19（ノード19-AP間）
        fairleader_tension = segment_tensions[0]  # FP側の張力
        anchor_tension = segment_tensions[-1] # AP側の張力
        tension_data.append([t, fairleader_tension, anchor_tension])
