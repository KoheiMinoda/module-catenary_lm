...
def create_line_animation(node_data, use_1sec_intervals=True):
    ...
    ax.set_title('Mooring Chain Animation', fontsize=14)
    ...
    # セグメント別ライン描画用オブジェクトを準備（全て青色）
    segment_lines = []
    
    for seg_id in range(20):  # 20セグメント
        line, = ax.plot([], [], color='blue', linewidth=4, alpha=0.9)
        segment_lines.append(line)
        print(f"セグメント {seg_id}: Steel Chain (blue)")
    
    # 凡例用のダミーライン（Chain のみ）
    ax.plot([], [], color='blue', linewidth=4, label='Steel Chain', alpha=0.9)
...
def save_animation(anim, filename='chain_mooring_animation.mp4'):
    try:
...
def main():
    print("=== Chain Mooring Line Animation ===")
    ...
    print("\n=== Animation Features ===")
    print("- Blue lines: Steel chain segments")
    ...
    if save_choice == 'y':
        filename = input("Enter filename (default: chain_mooring_animation.mp4): ").strip()
        if not filename:
            filename = 'chain_mooring_animation.mp4'
