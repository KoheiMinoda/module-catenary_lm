...
MU_LATERAL = 0.5
MU_AXIAL = 0.0
V_SLIP_TOL_LATERAL = 1.0e-3
V_SLIP_TOL_AXIAL = 1.0e-3
SMOOTH_CLEARANCE = 0.0005
SMOOTH_EPS = 0.0001
...
def reynolds_drag_correction(velocity, diameter, kinematic_viscosity, surface_roughness=0.001, chain_factor=1.9):
    if velocity < 1e-8:
        return 1.0
    
    Re = abs(velocity) * diameter / kinematic_viscosity
...
def calculate_advanced_morison_forces(nodes, segments, t):
    ...
    for seg in segments:
        ...
        cd_correction = reynolds_drag_correction(
            u_rel_mag, 
            LineDiameter, 
            KINEMATIC_VISCOSITY
        )
        cd_n = CD_NORMAL * cd_correction
        cd_t = CD_AXIAL * cd_correction
...
def compute_acc(nodes, segments, t):
    ...
    for k, node in enumerate(nodes):
        ...
        m_eff = effective_masses[k] # LineDryMass
        a_k = F_tot / m_eff
        node["acc"] = a_k.copy()
        acc.append(a_k)
