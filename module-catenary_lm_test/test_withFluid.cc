...
class ModuleCatenaryLM
    ...
    private:
        ...
        // 新規追加：流体力パラメータ
        doublereal rho_water;               // 海水密度 [kg/m³]
        doublereal cd_normal;               // 法線方向抗力係数
        doublereal cd_axial;                // 軸方向抗力係数
        doublereal cm_normal;               // 法線方向慣性係数
        doublereal ca_normal;               // 法線方向付加質量係数
        doublereal wave_height;             // 波高 [m]
        doublereal wave_period;             // 波周期 [s]
        doublereal wave_direction;          // 波向 [deg]
        doublereal water_depth;             // 水深 [m]
        ...
        Vec3 GetWaveVelocityAcceleration(const Vec3& position, doublereal time) const;
        Vec3 ComputeFluidForce(const Vec3& pos1, const Vec3& pos2, const Vec3& vel1, const Vec3& vel2, doublereal seg_length, doublereal time) const;
};
...
ModuleCatenaryLM::ModuleCatenaryLM(
    ...
    : Elem(uLabel, flag(0)), UserDefinedElem(uLabel, pDO), 
        ...
        rho_water(1025.0), cd_normal(1.2), cd_axial(0.008),
        cm_normal(2.0), ca_normal(1.0),
...
// ---- 流体の加速度 ----
Vec3 ModuleCatenaryLM::GetWaveVelocityAcceleration(const Vec3& position, doublereal time) const
{
    if (wave_height < 1e-6) {
        return Vec3(0.0, 0.0, 0.0);
    }
    ...
    doublereal phase = k * x * std::cos(wave_dir_rad) - omega * time;
    
    doublereal depth_factor = std::cosh(k * (water_depth - depth_from_surface)) / std::sinh(k * water_depth);
    
    doublereal ax = amplitude * omega * omega * depth_factor * std::sin(phase) * std::cos(wave_dir_rad);
    doublereal ay = amplitude * omega * omega * depth_factor * std::sin(phase) * std::sin(wave_dir_rad);
    doublereal az = 0.0;
...
// ---- セグメント流体力計算 ----
Vec3 ModuleCatenaryLM::ComputeFluidForce(const Vec3& pos1, const Vec3& pos2, const Vec3& vel1, const Vec3& vel2, doublereal seg_length, doublereal time) const
{
    Vec3 pos_mid = (pos1 + pos2) * 0.5;
    Vec3 vel_mid = (vel1 + vel2) * 0.5;
    Vec3 seg_vector = pos2 - pos1;
    
    if (seg_length < 1e-12) {
        return Vec3(0.0, 0.0, 0.0);
    }
    
    Vec3 t_vec = seg_vector / seg_length;
    Vec3 fluid_acc = GetWaveVelocityAcceleration(pos_mid, time);
    
    // Froude-Krylov力
    doublereal volume = M_PI * (line_diameter/2.0) * (line_diameter/2.0) * seg_length;
    Vec3 F_FK = fluid_acc * (rho_water * volume * cm_normal);
    
    // 相対速度（流体速度は簡略化のため 0 と仮定）
    Vec3 vel_rel = -vel_mid;
    
    doublereal vel_t = vel_rel.Dot(t_vec);
    Vec3 vel_t_vec = t_vec * vel_t;
    Vec3 vel_n_vec = vel_rel - vel_t_vec;
    
    doublereal vel_n_mag = vel_n_vec.Norm();
    doublereal vel_t_mag = std::abs(vel_t);
    
    // 抗力計算
    Vec3 F_drag(0.0, 0.0, 0.0);
    
    // 法線方向抗力
    if (vel_n_mag > 1e-6) {
        doublereal area_n = line_diameter * seg_length;
        doublereal F_drag_n_mag = 0.5 * rho_water * cd_normal * area_n * vel_n_mag * vel_n_mag;
        F_drag += vel_n_vec * (F_drag_n_mag / vel_n_mag);
    }
    
    // 軸方向抗力
    if (vel_t_mag > 1e-6) {
        doublereal area_t = M_PI * (line_diameter/2.0) * (line_diameter/2.0);
        doublereal F_drag_t_mag = 0.5 * rho_water * cd_axial * area_t * vel_t_mag * vel_t_mag;
        F_drag += vel_t_vec * (F_drag_t_mag * (vel_t > 0 ? 1.0 : -1.0) / vel_t_mag);
    }
    
    // 付加質量力（構造加速度部分は実効質量に含まれるため流体加速度部分のみ）
    Vec3 F_AM = fluid_acc * (rho_water * volume * ca_normal);
    
    return F_FK + F_drag + F_AM;
}
...
void ModuleCatenaryLM::UpdateVirtualNodes(doublereal dt)
{
    ...
    for (unsigned int i = 0; i < virtual_nodes.size(); ++i) {
        ...
        if (i == 0) {
            // フェアリーダーとの接続
            Vec3 F_axial_prev = ComputeAxialForce(
        ...
        if (i == virtual_nodes.size() - 1) {
            // アンカーとの接続
            Vec3 F_axial_next = ComputeAxialForce(
        ...
        // 前のセグメントの流体力
        if (i == 0) {
            F_fluid += ComputeFluidForce(
                fairlead_pos, virtual_nodes[i].position, 
                fairlead_vel, virtual_nodes[i].velocity, 
                segment_length, simulation_time
            ) * 0.5;
        } else {
            F_fluid += ComputeFluidForce(
                virtual_nodes[i-1].position, virtual_nodes[i].position,
...
void ModuleCatenaryLM::ComputeLumpedMassForces(Vec3& F_total, Vec3& M_total)
{
    ...
    F_total = (F_axial + F_fluid) * ramp_factor;
    M_total = Vec3(0.0, 0.0, 0.0);
}
